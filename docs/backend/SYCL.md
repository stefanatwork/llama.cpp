...
| GGML_SYCL_USE_HOST_USM | 0 (default) or 1 | Allocate backend buffers in SYCL host USM and treat them as shared CPU/GPU memory. This is intended for integrated GPUs with shared memory so llama.cpp can avoid duplicating buffers and skip explicit host<->device copies. Requires device support for `sycl::aspect::usm_host_allocations`; enabling it on dGPUs is possible but usually slower than device memory. |
