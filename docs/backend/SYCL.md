...
| GGML_SYCL_USE_HOST_USM | unset (auto), 0, or 1 | Allocate backend buffers in SYCL host USM and treat them as shared CPU/GPU memory. If unset, llama.cpp now auto-enables this mode when an integrated/shared-memory GPU with `sycl::aspect::usm_host_allocations` is detected. Use 0 or 1 to explicitly override auto-detection. Enabling on dGPUs is possible but usually slower than device memory. |
