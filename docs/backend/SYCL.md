...
| GGML_SYCL_USE_HOST_USM | unset (auto), 0, or 1 | Allocate backend buffers in SYCL shared USM (`sycl::malloc_shared`) to share pointers between CPU and GPU. If unset, llama.cpp auto-enables this mode when an integrated/shared-memory GPU with `sycl::aspect::usm_shared_allocations` is detected. Use 0 or 1 to explicitly override auto-detection. On dGPUs this can trigger on-demand page migration and is not always faster than device memory. |
